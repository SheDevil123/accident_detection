...
def idk():
    ...
    pywhatkit.sendwhatmsg("+916374795827","person on road \n bike pedestrian collision",h,m)

def msg1():
    pywhatkit.sendwhatmsg_to_group("C1XZNm9Vfdn5kmU7L32MTI",f"person on road \nbike pedestrian collision\n location:{g.latlng}\ndate and time: {curr}\nlocation: {geolocator.geocode(str(g.latlng[0])+','+str(g.latlng[1]))}",h,m)
        ...
        for j in bike:
            ...
            if dist(loc,mid(j[0]))<30:
                ...
                if t<=0:
                    ...
                    h=int(time.strftime('%H'))
                    m=int(time.strftime('%M'))+1
                    msg=f"person on road \nbike pedestrian collision\n location:{g.latlng}\ndate and time: {curr}\nlocation: {geolocator.geocode(str(g.latlng[0])+','+str(g.latlng[1]))}"
                    t1=threading.Thread(target=msg1,args=(msg,))
                    t1.start()
                t-=1
                break
...
# cv2.imshow("idk",crash_frame)
# cv2.waitKey(0)
cv2.imwrite("crash_pic.jpg",crash_frame)
...
#     t1=threading.Thread(target=msg1,args=(msg,))
    #pywhatkit.sendwhatmsg_to_group("C1XZNm9Vfdn5kmU7L32MTI",f"person on road \nbike pedestrian collision\n location:{g.latlng}\ndate and time: {curr}\nlocation: {geolocator.geocode(str(g.latlng[0])+','+str(g.latlng[1]))}",h,m)
    #pywhatkit.sendwhatmsg("+917667472476",f"person on road \nbike pedestrian collision\n location:{g.latlng}\ndate and time: {curr}\nlocation: {geolocator.geocode(str(g.latlng[0])+','+str(g.latlng[1]))}",h,m)
