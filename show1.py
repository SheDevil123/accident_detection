...
if crash_bool:
    geolocator = Nominatim(user_agent="show1")
    g = geocoder.ip('me')
    h=int(time.strftime('%H'))
    m=int(time.strftime('%M'))+1
    #pywhatkit.sendwhatmsg_to_group("Fg8YBFqGJ07Jiqsgqdrq94",f"person on road \nbike pedestrian collision\n location:{g.latlng}\ndate and time: {curr}\nlocation: {geolocator.geocode(str(g.latlng[0])+','+str(g.latlng[1]))}",h,m,18,True)
    pywhatkit.sendwhatmsg("+916374795827",f"person on road \nbike pedestrian collision\n location:{g.latlng}\ndate and time: {curr}\nlocation: {geolocator.geocode(str(g.latlng[0])+','+str(g.latlng[1]))}",h,m,18,True)
